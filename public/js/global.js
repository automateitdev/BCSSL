...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        for (var i = 0; i < data.length; i++) {
          op += '<option value="' + data[i].id + '">' + data[i].group_name + "</option>";
        }
    ...
    if ($(this).prop("checked") == true) {
      $(".single_check_sum").each(function () {
        ...
        if (self.prop("checked") == true) {
          var totalamount = self.data("totalamount");
          total_amount = total_amount + parseFloat(totalamount);
        }
      });
      total_amount > 0 ? grand_total_fadeIn(total_amount) : grand_total_fadeOut();
    } else if ($(this).prop("checked") == false) {
      // grand_total_fadeOut();
      $(".single_check_sum").each(function () {
        var self = $(this);
  ...
  $(document).on("click", ".adminNavbarDropdown", function (e) {
    ...
    self.next(".adminNavbarDropdown").toggleClass("show").attr("data-bs-popper", function (index, attr) {
